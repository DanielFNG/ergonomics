function [mos, wmos, wcen, forces, timesteps] = calculateMoS(model_path, solution_path)
    ...
    wcen = zeros(solution.NFrames, 1);
    
    % Initialise forces cell array
    n_forces = length(force_strings);
    forces = cell(n_forces, 1);
    for i = 1:n_forces
        forces{i} = [];
    end
    ...
    for i = 1:solution.NFrames
        ...
        for j = 1:n_forces
            ...
            % Store force value
            these_forces = [force_value.get(0); force_value.get(1); force_value.get(2)];
            forces{j} = [forces{j}, these_forces];
        ...
        if ~isempty(polygon_points)
            ...
            wmos(i) = sqrt((wcentroid_x - com(1))^2 + (wcentroid_y - com(3))^2);
